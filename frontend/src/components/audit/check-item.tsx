...
const statusConfig: Record<CheckStatus, { icon: string; bg: string; text: string; border: string }> = {
  pass: { icon: "\u2713", bg: "bg-emerald-500/20", text: "text-emerald-400", border: "" },
  warn: { icon: "!", bg: "bg-yellow-500/20", text: "text-yellow-400", border: "border-yellow-500/30 hover:border-yellow-500/50" },
  fail: { icon: "\u2717", bg: "bg-red-500/20", text: "text-red-400", border: "border-red-500/30 hover:border-red-500/50" },
  info: { icon: "i", bg: "bg-blue-500/20", text: "text-blue-400", border: "" },
};
...
export function CheckItem({ check }: CheckItemProps) {
  ...
  const borderClass = isActionable ? config.border : "border-slate-800/50";
    ...
    >
      ...
      {expanded && isActionable && (
        <div className="border-t border-slate-800/50 px-4 py-3">
          ...
          <div className={`rounded-md p-3 ${check.status === "fail" ? "bg-red-500/10" : "bg-yellow-500/10"}`}>
